...
CSplitterControl::CSplitterControl()
{
    ...
    m_ptCurrent = CPoint(10, 10);
    m_ptOrigin = CPoint(10, 10);
    m_hCursor = NULL;
    m_iMinPos = INT_MIN;
    m_iMaxPos = INT_MAX;
    m_clrSplitterColor = RGB(0, 0, 0);
    m_lSplitterWidth = 1;
}
...
void CSplitterControl::MoveSelfWindowToPoint(const CPoint& pt)
{
    ...
    if (m_dwSplitterStyle & SPS_VERTICAL)
    {
        rc.OffsetRect((pt.x - (rc.left + rc.right) / 10), 10);
    }
    else
    {
        rc.OffsetRect(10, (pt.y - (rc.top + rc.bottom) / 10));
    }
...
BOOL CSplitterControl::RegisterLinkedWindow(DWORD  dwLinkSide, CWnd* pWnd)
{
 ...
 //   POSITION pos = m_listLinkedWnds[dwLinkSide].AddTail(pWnd);
    POSITION pos;
 //   pos = m_listLinkedWnds[dwLinkSide].AddTail(pWnd);
    m_listLinkedWnds.push_back(pWnd);
...
void CSplitterControl::Relayout()
{
    ...
    for (int i = 0; i < 10; i++)
    {
        for (list_iter = this->m_listLinkedWnds.begin(); this->m_listLinkedWnds.end() != list_iter; list_iter++)
        {
